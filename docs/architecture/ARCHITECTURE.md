...
│   ├── version.js        # requireMinVersion — rejects outdated app versions with 426
...
│   └── version.js        # getMinVersion (cached) & isBelowMinVersion comparator
└── routes/               # one file per resource (auth, products, customers, personnel,
                          # orders, incoming, stations, tickets, audit, dashboard, version)
server/db/
...
Every route file mounts `router.use(requireAuth)` except `auth.js` (where only `/me` is guarded) and `version.js`,
so **all `/api/v1/*` endpoints require auth except `POST /api/v1/auth/login`, `POST /api/v1/auth/logout`, and `GET /api/v1/version`**.
...
│                         #   injects Bearer token & X-App-Version/Build headers, handles 401 & 426
...
├── version/              # appVersion.js — getAppInfo, isBelowMinVersion, openPlayStore
├── components/           # layout, ui primitives, version/ (VersionGate, RequiredUpdateScreen)
...
7. **Minimum-Version Enforcement (migration 047):**
   - On app launch and foreground return, the client polls `GET /api/v1/version` when online.
   - If the installed app is below `app_settings.min_version`, normal app use is immediately blocked by `<RequiredUpdateScreen />` with an Update button linking to Google Play Store internal distribution.
   - If offline, existing offline operations continue until reconnect.
   - Server enforces `requireMinVersion` across all protected routes (`requireAuth`) and `POST /login`, rejecting outdated clients with `426 Upgrade Required` (`code: 'update_required'`).
   - Enforcement is dormant by default (`min_version IS NULL`).

---

## Database Security & Row Level Security (RLS)

Per migrations `045_enable_rls.sql` and `047_app_settings_min_version.sql` and [ADR 0018](../adr/0018-supabase-rls-lockdown.md), Row Level Security (RLS) is enabled across all 17 tables:
